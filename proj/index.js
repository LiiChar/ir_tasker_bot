...
const token = '5910779899:AAFbwW2f7sceZhWnYXERy_HUAl3HKmUL8bo' //'6284064559:AAHxBH9ohl-gXbyfhxwGm0I5Td9m-KOsHJo' //'5712309589:AAFyq_C9l39rDgsgApuy80Cfb0dlshyFZO8'
...
bot.on('callback_query', (query) => {
    ...
    else if (query.data === 'vacation_aprove') {
        (async function vacation(text, id) {
    ...
    else if (query.data === 'vacation_!aprove') {
        (async function vacation(text, id) {
            const vacation_id = await text.match(/\[#(.*)\]/)[1];
    ...
    else if (query.data === 'day_!aprove') {
        (async function vacation(text, id) {
            const vacation_id = await text.match(/\[#(.*)\]/)[1];
...
bot.onText(/\/2048/, (msg) => {
    try {
        bot.sendGame(msg.chat.id, 'game_cube');

        bot.on('callback_query', function onCallbackQuery(callbackQuery) {
            bot.answerCallbackQuery(callbackQuery.id, { url: 'https://2048-litavanchik-gmailcom.vercel.app/' });
        });
    } catch (e) {
        console.log(e);
    }
